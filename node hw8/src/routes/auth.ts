...
import { User } from "../models/users.entity.ts";
...
userRouter.post("/register", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!username || !email || !password || !confirmPassword) {
      return res.status(400).json({ message: `Please fill requirments.` });
    }
    ...
    return res.status(201).json({ message: "Success!", token: `Bearer ${token}` });
  } catch (error) {
    return res.status(500).json({ message: "Internal server error" });
  }
});
